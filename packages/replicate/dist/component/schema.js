import { cursorValidator, profileValidator } from "./shared/index.js";
...
var schema_default = defineSchema({
	devices: defineTable({
		...
		lastSeen: v.number(),
		approved: v.boolean()
	}).index("by_user", ["collection", "userId"]).index("by_device", [
		"collection",
		"userId",
		"deviceId"
	]),
	...
	docKeys: defineTable({
		...
		wrappedKey: v.bytes(),
		created: v.number()
	}).index("by_document", ["collection", "document"]).index("by_user_doc", [
		"collection",
		"userId",
		"document"
	]),
