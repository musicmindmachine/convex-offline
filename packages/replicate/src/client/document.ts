...
import type { EditorBinding, ProseOptions } from "$/client/collection";
import {
	getStableAnonColor,
	getStableAnonName,
	hashStringToNumber,
...
export interface DocumentPresenceConfig {
...
export interface DocumentPresenceProvider extends DocumentPresence {
	awareness: Awareness;
	destroy(): void;
}

export function createDocumentPresence(config: DocumentPresenceConfig): DocumentPresenceProvider {
	const {
		convexClient,
		api,
		document,
	...
	const resolvedUser = user ?? userGetter?.();
